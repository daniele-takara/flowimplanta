...
function _legacyComputeMacroSchedule(overrides, answersMap, project, savedActivities, phaseOverridesMap) {
  ...
  SCHEDULE_TASKS.forEach(task => {
    ...
    // Pular atividades inativadas neste projeto (detecção pelo marcador, independente do status)
    const isInactivated = (saved.history_observations || "").includes("[INATIVADO]");
