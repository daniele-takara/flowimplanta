  ...
  const loadData = async () => {
    ...
    try {
      if (isMock) {
        ...
        const [proj, ph, ac, sc, rp, ap] = await Promise.all([
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 p-8 bg-slate-50">
        <div className="max-w-6xl mx-auto">
          {activeTab === "overview" && (
            <OverviewTab project={project} phases={phases} />
          )}
          {activeTab === "scope" && (
            <ScopeTab scopeItems={scopeItems} projectId={id} onRefresh={loadData} />
          )}
          {activeTab === "schedule" && (
            <ScheduleTab phases={phases} activities={activities} projectId={id} onRefresh={loadData} />
          )}
          {activeTab === "status" && (
            <StatusReportTab reports={reports} projectId={id} onRefresh={loadData} />
          )}
          {activeTab === "actions" && (
            <ActionPlanTab actions={actions} projectId={id} onRefresh={loadData} />
          )}
        </div>
      </div>
    </div>
