...
// Auto-derive deliverables from modules + scope answers
function deriveDeliverables(project, scopeItems) {
  const deliverables = [];
...
export default function TAPTab({ project, scopeItems, documents, projectId, onRefresh }) {
  ...
  const derivedDeliverables = deriveDeliverables(project, scopeItems);

  const set = (f, v) => setForm(p => ({ ...p, [f]: v }));

  const handleSave = async () => {
    setSaving(true);
    const payload = { ...form, project_id: projectId, doc_type: "TAP", deliverables: derivedDeliverables };
    if (tap?.id) {
  ...
  const handleExportPDF = () => {
    const content = buildPDFContent(project, form, derivedDeliverables);
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400">Documento formal de abertura do projeto — preenchido automaticamente com base nos Dados Iniciais e Escopo Técnico</p>
        </div>
      ...
      {editing && (
        <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-100 rounded-lg mb-5 text-sm text-blue-700">
          ...
          <span>Os campos abaixo foram pré-preenchidos com base nos Dados Iniciais e no Escopo Técnico do projeto. Revise e ajuste conforme necessário.</span>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          {editing
            ? <textarea className={inputClass} rows={3} value={form.objective} onChange={e => set("objective", e.target.value)} />
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            {editing
              ? <textarea className={inputClass} rows={4} value={form.scope_description} onChange={e => set("scope_description", e.target.value)} />
              : <p className="text-sm text-slate-700 whitespace-pre-wrap">{form.scope_description || "—"}</p>
            }
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Fora do Escopo</h3>
            {editing
              ? <textarea className={inputClass} rows={4} value={form.out_of_scope} onChange={e => set("out_of_scope", e.target.value)} />
              : <p className="text-sm text-slate-700 whitespace-pre-wrap">{form.out_of_scope || "—"}</p>
            }
          </div>
        </div>

        {/* Deliverables — auto-derived */}
        <div className="bg-white rounded-xl border border-slate-200 p-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            {editing
              ? <textarea className={inputClass} rows={4} value={form.assumptions} onChange={e => set("assumptions", e.target.value)} />
              : <p className="text-sm text-slate-700 whitespace-pre-wrap">{form.assumptions || "—"}</p>
            }
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Restrições</h3>
            {editing
              ? <textarea className={inputClass} rows={4} value={form.restrictions} onChange={e => set("restrictions", e.target.value)} />
              : <p className="text-sm text-slate-700 whitespace-pre-wrap">{form.restrictions || "—"}</p>
            }
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            {editing
              ? <textarea className={inputClass} rows={3} value={form.risks_summary} onChange={e => set("risks_summary", e.target.value)} />
              : <p className="text-sm text-slate-700 whitespace-pre-wrap">{form.risks_summary || "—"}</p>
            }
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Critérios de Sucesso</h3>
            {editing
              ? <textarea className={inputClass} rows={3} value={form.success_criteria} onChange={e => set("success_criteria", e.target.value)} />
...
<h2>Escopo</h2>
<p>${form.scope_description || '—'}</p>

<h2>Fora do Escopo</h2>
<p>${form.out_of_scope || '—'}</p>

<h2>Entregáveis</h2>
<ul>${deliverables.map(d => `<li>${d}</li>`).join('')}</ul>

<h2>Premissas</h2>
<p>${form.assumptions || '—'}</p>

<h2>Restrições</h2>
<p>${form.restrictions || '—'}</p>

<h2>Resumo de Riscos</h2>
<p>${form.risks_summary || '—'}</p>
